...
ALTER PROCEDURE [LTV].[Build_DataPanel_Predictions]
AS
BEGIN
...
WHERE syn_active_Fact_SF_Opportunity.Opportunity_Last_Modified_Datetime <= @CutPoint
          ...
          AND DATEDIFF(DAY, syn_active_Fact_SF_Opportunity.Opportunity_Last_Modified_Datetime, @CutPoint) <= 2*@ActivityDays
...
SELECT Opportunity_SF_Account_ID,
       ...
       Opportunity_Expected_Annual_Revenue_Amount_USD = ISNULL(SUM(Opportunity_Expected_Annual_Revenue_Amount_USD), 0),
       Opportunity_Forecast_Annual_Revenue_Amount_USD = ISNULL(SUM(Opportunity_Forecast_Annual_Revenue_Amount_USD), 0)
...
--SELECT * FROM #SalesForceData_Opportunity
    ...
    /**************************************************** DataPanel ***************************************/
    IF OBJECT_ID('[Dev_Predict_DB].[LTV].[DataPanel_Prediction]') IS NOT NULL
       DROP TABLE [Dev_Predict_DB].[LTV].[DataPanel_Prediction];
    ...
    SELECT Account_Holder_ID = BaseView.Account_Holder_ID,
		   ...
		   OpportunityMonthlyVolumeCommitmentAmountUSD = ISNULL(SalesForceData_Opportunity.Opportunity_Monthly_Volume_Commitment_Amount_USD,-1),
		   OpportunityExpectedAnnualVolumeAmountUSD = ISNULL(SalesForceData_Opportunity.Opportunity_Expected_Annual_Volume_Amount_USD, -1),
		   OpportunityExpectedAnnualRevenueAmountUSD = ISNULL(SalesForceData_Opportunity.Opportunity_Expected_Annual_Revenue_Amount_USD, -1),
		   OpportunityForecastAnnualRevenueAmountUSD = ISNULL(SalesForceData_Opportunity.Opportunity_Forecast_Annual_Revenue_Amount_USD, -1),
           ...
           Y = ISNULL(Y.LTV, 0)
        INTO [Dev_Predict_DB].[LTV].[DataPanel_Prediction]
