tryCatch(
  {
    ...
    NumberOfCores<-round(parallel:::detectCores()/9)+1
    #NumberOfCores<-2
    ...
    DataPanelPredict<-sqlQuery(channel = channel,query = "select * from [Predict_DB].[LTV].[DataPanel_Prediction]")
    odbcClose(channel)
    ...
    ##Delete All DataPredict for the new one --------------------------------------------------------------------------------------
    file.remove(list.files(path = "Data/DataPredict/",full.names = TRUE))
    ...
    for(Segment in unique(DataPanelPredict$Segment)){
      ...
      save(DataPredict,compress = FALSE,
           file = gsub(x =  "Data/DataPredict/LTV - DataPredict - Segment.RData",pattern = "Segment",replacement = Segment))
    ...
    Output<-ddply(.data =  data.frame(Segment=AllPredictSegments),
                  ...
                  .fun = function(Segment){
                    ...
                    load(file = gsub(x =  "Data/DataPredict/LTV - DataPredict - Segment.RData",pattern = "Segment",replacement = as.character(Segment$Segment[1])))
                    #load(file = gsub(x =  "Data/DataPredict/LTV - DataPredict - Segment.RData",pattern = "Segment",replacement = Segment))
    ...
    sqlQuery(channel,"IF OBJECT_ID('[Predict_DB].[dbo].[TempOutputLTV]', 'U') IS NOT NULL
             DROP TABLE [Predict_DB].[dbo].[TempOutputLTV]")
    
    TempOutputLTV<-data.frame(ID=Output$Account_Holder_ID,
    ...
    sqlSave(channel,TempOutputLTV,rownames=F,varTypes = varTypes)
    ...
    sqlQuery(channel, "
             ...
             select TempOutputLTV.* from [Predict_DB].[dbo].[TempOutputLTV] as TempOutputLTV
    ...
    VALUES ('LTV', 'LTV - New Script - Prediction', 'Output the predictions', 'Output the predictions script ran successfully',1,'2017-01-02 00:00:00.000');"
  ...
  ,error=function(err){
    ...
    Alerts_conn<-odbcDriverConnect('SERVER=localhost;DRIVER={SQL Server};DATABASE=Predict_DB;trusted_connection=true')
    QueryString<-"INSERT INTO [Predict_DB].[dbo].Alerts (MasterModel, Model, Step,Description,Successful,DateRunnigTheStep)
    VALUES ('LTV', 'LTV - New Script - Prediction', 'Output the predictions', 'Output the predictions script not ran successfully',0,'2017-01-02 00:00:00.000');"
    sqlQuery(channel = Alerts_conn,
             query = gsub(x = QueryString,pattern = '2017-01-02 00:00:00.000',replacement = as.character.Date(Sys.time())))
    odbcClose(Alerts_conn)
  }
