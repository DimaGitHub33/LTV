...
sql = "select Account_Holder_ID,Season,PrimaryVertical,Seniority,Country_Name from [Dev_Predict_DB].[LTV].[DataPanel_Model_TrainTest]"
...
## ---------------------------------------------------------------------------------------------------------------------------------
for filename in os.listdir('Data Pickle/DataModel/'):
    os.remove(os.getcwd() + '/Data Pickle/DataModel/' + filename)
...
for Segment in DataToSplit['Segment'].unique():
    ...
    DataModel = pd.DataFrame(pd.read_sql("""select DPMTT.* 
                                            from [Dev_Predict_DB].[LTV].[DataPanel_Model] as DPM 
                                            inner join #SegmentTable as SegmentTable
                                            on DPM.Account_Holder_ID = SegmentTable.Account_Holder_ID AND
                                            DPM.Season = SegmentTable.Season""", conn))
    #len(DataModel)  # 5230765
    
    ## Write the table
    Path = 'Data Pickle/DataModel/LTV - DataModel - Segment.pckl'
...
def RunModelParallel(Data):
    ...
    df = pd.DataFrame(data={'Account_Holder_ID': DataModel['Account_Holder_ID'].astype(int),
                            'Model': 'LTV - New Script',
                            'Segment': Segment,
                            'Season': DataModel['Season'].astype(str),

                            'Seniority': DataModel['Seniority'],
                            'Country': DataModel['Country_Name'].astype(str),
                            'PrimaryVertical': DataModel['PrimaryVertical'].astype(str),
                            
                            'Predict': DataModel['Predict'],
                            'Predict_calibrated': DataModel['Predict_calibrated'],
                            'LMPredict': DataModel['LMPredict'],
                            'ThisYearLTV': DataModel['ThisYearLTV'],
...
# -----------------------------------------------------------------------------------------------------------------------------
## Files Delete in Data Pickle/DataModel--------------------------------------------------------------------------------------------
for filename in os.listdir('Models - Python Pickle/'):
    os.remove(os.getcwd() + '/Models - Python Pickle/' + filename)
...
AllModelSegments = os.listdir('Data Pickle/DataModel/')
...
ModelOut = DPCM.groupby('Segments').apply(RunModelParallel,,meta={'Account_Holder_ID': 'int',
                                                                             'Model': 'str',
                                                                             'Segment': 'str',
                                                                             'Season': 'str',
...
print(EndModel - StartModel)##5:07:39.525278
...
for PrimaryVertical in ('All',*ModelOut['PrimaryVertical'].unique()):
    # print(PrimaryVertical)
    
    ##Sub Data per Vertical
    if PrimaryVertical=='All' :
      SubData = ModelOut.reset_index()
    else:
      SubData = ModelOut.loc[ModelOut['PrimaryVertical'] == PrimaryVertical, :].reset_index()
    ...
    CheckingTheModel = pd.concat([CheckingTheModel,
        pd.DataFrame(data={'PrimaryVertical':PrimaryVertical,
                           ...
                           'PercentActualFromTotal': round(100*sum(SubData['Actual'])/sum(ModelOut['Actual']), 4),
