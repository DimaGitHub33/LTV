...
sql = "select Account_Holder_ID,Season,PrimaryVertical,Seniority,Country_Name from [Dev_Predict_DB].[LTV].[DataPanel_FTL_Prediction]"
...
## ---------------------------------------------------------------------------------------------------------------------------------
## Files Delete in Data Pickle -----------------------------------------------------------------------------------------------------
## ---------------------------------------------------------------------------------------------------------------------------------
for filename in os.listdir('Data Pickle/DataPredictFTL/'):
    os.remove(os.getcwd() + '/Data Pickle/DataPredictFTL/' + filename)
    

## Save The New Data Segments ------------------------------------------------------------------------------------------------------
Start = datetime.now()
# conn = pyodbc.connect('DRIVER={SQL Server};SERVER=HRZ-DWHSS-PRD;DATABASE=Predict_DB;Trusted_Connection=yes')
conn = pyodbc.connect('SERVER=HRZ-DWHSS-PRD;DRIVER={SQL Server};DATABASE=Dev_Predict_DB;UID=bipredict;PWD=CokeZero!')
cursor = conn.cursor()
for Segment in DataPredictToSplit['Segment'].unique():
    ...
    DataPredict = pd.DataFrame(pd.read_sql("""select DPFP.* 
                                            from [Dev_Predict_DB].[LTV].[DataPanel_FTL_Prediction] as DPFP 
                                            inner join #SegmentTable as SegmentTable
                                            on DPFP.Account_Holder_ID = SegmentTable.Account_Holder_ID AND
                                            DPFP.Season = SegmentTable.Season""", conn))
    #len(DataPredict)  # 5230765
    
    ## Write the table
    Path = 'Data Pickle/DataPredictFTL/LTV - FTL - DataPredict - Segment.pckl'
...
# -----------------------------------------------------------------------------------------------------------------------------
##Prediction function
def RunFTLPredictionsParallel(Data):
    #print(Data.iloc[0,0])
...
# -----------------------------------------------------------------------------------------------------------------------------
## Start Time
StartFTLPredictions = datetime.now()


##Taking all the Data Pickles
os.chdir("E:/LTV - New Script")
os.getcwd()
AllPredictionsSegments = os.listdir('Data Pickle/DataPredictFTL/')
AllPredictionsSegments = [*map(lambda x: x.replace('.pckl',"", 1).replace('LTV - FTL - DataPredict - ',"", 1), AllPredictionsSegments)]
...
Output = DaskSegments.groupby('Segments').apply(RunFTLPredictionsParallel,meta={'Account_Holder_ID': 'int',
                                                                             'Model': 'str',
                                                                             'Segment': 'str',
                                                                             'Season': 'str',
...
## End Time
EndFTLPredictions = datetime.now()
print(StartFTLPredictions.strftime("%Y-%m-%d %H:%M:%S"))
print(EndFTLPredictions.strftime("%Y-%m-%d %H:%M:%S"))
print(EndFTLPredictions - StartFTLPredictions)##0:23:35.034289

del StartFTLPredictions,EndFTLPredictions
...
## Drop the table and close the connection
cursor.execute("""DROP TABLE IF Exists [Dev_Predict_DB].[LTV].[TempOutputLTV_FTL]""")
conn.commit()

## Prepare the temporary table
TempOutputLTV_FTL = pd.DataFrame(
...
TempOutputLTV_FTL.Model = TempOutputLTV_FTL.Model.str.slice(start=0, stop=29).astype(str)
TempOutputLTV_FTL.PrimaryVertical = TempOutputLTV_FTL.PrimaryVertical.str.slice(start=0, stop=29).astype(str)
TempOutputLTV_FTL.ChurnCategory = TempOutputLTV_FTL.ChurnCategory.str.slice(start=0, stop=29).astype(str)
TempOutputLTV_FTL.LocalRank = TempOutputLTV_FTL.LocalRank.astype(int)
TempOutputLTV_FTL.GlobalRank = TempOutputLTV_FTL.GlobalRank.astype(int)
TempOutputLTV_FTL.CurrentDate = TempOutputLTV_FTL.CurrentDate.astype('datetime64[ns]')
TempOutputLTV_FTL.UpdateDate = TempOutputLTV_FTL.UpdateDate.astype('datetime64[ns]')
TempOutputLTV_FTL.PredictionForDate = TempOutputLTV_FTL.PredictionForDate.astype('datetime64[ns]')
...
## TempOutputLTV_FTL
TempOutputLTV_FTL.to_sql(name='TempOutputLTV_FTL',
...
sql = """
        ...
        select TempOutputLTV_FTL.* from [Dev_Predict_DB].[LTV].[TempOutputLTV_FTL] as TempOutputLTV_FTL
        
        COMMIT TRANSACTION
     """
## insert TempOutputLTV_FTL to [Dev_Predict_DB].[dbo].[Output] 
cursor.execute(sql)
conn.commit()
