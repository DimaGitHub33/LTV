tryCatch(
  {
    ...
    DataPanelModel<-sqlQuery(channel = channel,query = "select * from [Predict_DB].[LTV].[DataPanel_Model]")
    odbcClose(channel)
    nrow(DataPanelModel)##389767
    ...
    ##Delete All DataPanels for the new one -------------------------------------------------------------------------------------
    file.remove(list.files(path = "Data/DataModel/",full.names = TRUE))
    ...
    for(Segment in as.character(unique(DataPanelModel$Segment))){
      ...
      save(DataModel,compress = FALSE,
           file = gsub(x =  "Data/DataModel/LTV - Data - Segment.RData",pattern = "Segment",replacement = Segment))
      
    }
    
    ##All Segments to work on
    AllModelSegments<-unique(DataPanelModel$Segment)
    rm(DataPanelModel)
    
    Start<-Sys.time()
    
    ##Delete All Models before creating new models
    file.remove(list.files(path = "Models/",full.names = TRUE))
    
    ##Create the Models
    doSNOW::registerDoSNOW(cl<-snow::makeCluster(NumberOfCores))
    MOutput<-ddply(.data =  data.frame(Segment=AllModelSegments),
                   .variables = "Segment",
                   .progress =  "text",
                   .parallel = TRUE,
                   .inform = TRUE,
                   ...
                   .fun = function(Segment){
                     ...
                     load(file = gsub(x =  "Data/DataModel/LTV - Data - Segment.RData",pattern = "Segment",replacement = as.character(Segment$Segment[1])))
                     #load(file = gsub(x =  "Data/DataModel/LTV - Data - Segment.RData",pattern = "Segment",replacement = Segment))
                     ...
                     # })
                     DataModel$Calibration<-Calibration[findInterval(x = DataModel$Predict, vec = Calibration$lag_value,left.open = T),c("Calibration")]
                     ...
                     # Output-------------------------------------------------------------
                     Out<-data.frame(Account_Holder_ID=DataModel$Account_Holder_ID,
                                     Model="LTV - New Script",
                                     Segment=DataModel$Segment,
                                     Season=DataModel$Season,
                                     
                                     Seniority=DataModel$Seniority,
                                     Country=DataModel$Country_Name,
                                     PrimaryVertical=DataModel$PrimaryVertical,
                                     
                                     Predict=DataModel$Predict,
                                     Predict_calibrated=DataModel$Predict_calibrated,
                                     
                                     Churn90DaysPredict=DataModel$Churn90DaysPredict,
                                     Actual=DataModel$Y)
                     
                     rm(list=setdiff(ls(), c("NewEnvironment","Segment","Out")))
                     #gc()
                     
                     save(NewEnvironment,
                          file = gsub(x =  "Models/LTV - Updated Script - Segment.RData",pattern = "Segment",replacement = Segment))
    ...
    # ##Delete All DataPnales for keeping space
    file.remove(list.files(path = "Data/DataModel/",full.names = TRUE))
    
    # ----------------------------------------------------------------------------------------------------------------------
    # ----------------------------------------------- Cumulative Volume percentage -----------------------------------------
    # ----------------------------------------------------------------------------------------------------------------------
    OrderPrediction<-order(MOutput$Predict_calibrated,decreasing = TRUE)
    Predicted<-MOutput$Predict_calibrated[OrderPrediction]
    Y_predict<-MOutput$Actual[OrderPrediction]
    ...
    ##Optimal
    lines(x = 100*(1:length(MOutput$Actual[order(MOutput$Actual,decreasing = TRUE)])/length(MOutput$Actual[order(MOutput$Actual,decreasing = TRUE)])),
          y = 100*cumsum(MOutput$Actual[order(MOutput$Actual,decreasing = TRUE)])/sum(MOutput$Actual[order(MOutput$Actual,decreasing = TRUE)]),col="green")
    ##Actual
    ...
    for(PrimaryVertical in c("All",unique(as.character(MOutput$PrimaryVertical)))){
      
      ##Sub Data per Vertical
      if(PrimaryVertical=='All'){
        SubData<-MOutput
      }else{
        SubData<-MOutput[which(MOutput$PrimaryVertical==PrimaryVertical),]
      }
      ...
      GroupMappe<-mean(100*(abs(GroupMappe$Mean_Predict_calibrated-GroupMappe$Mean_Actual)+1)/(GroupMappe$Mean_Actual+1))
      
      ##RMSE
      RMSE<-sqrt(mean((SubData$Actual - SubData$Predict_calibrated)^2))
      
      ##MSE without Outliers
      RMSE_WithoutOutliers<-sqrt(mean((SubData$Actual[SubData$Actual<=quantile(SubData$Actual,0.9999)] - SubData$Predict_calibrated[SubData$Actual<=quantile(SubData$Actual,0.9999)])^2))
      
      ##MedAE
      MedAE<-median(abs(SubData$Actual - SubData$Predict_calibrated))
      ...
      ##60% Difference
      Difference60Percent<-mean(((SubData$Predict_calibrated+1)/(SubData$Actual+1))<=0.6  | ((SubData$Predict_calibrated+1)/(SubData$Actual+1))>=1.6)
      
      ##Personal Mappe
      PersonalMappe<-mean((abs(SubData$Predict_calibrated-SubData$Actual)+1)/(abs(SubData$Actual)+1))
      ...
      CheckingTheModel<-rbind(CheckingTheModel,data.frame(PrimaryVertical,
                                                          ...
                                                          PercentActualFromTotal=round(100*sum(SubData$Actual)/sum(MOutput$Actual),4),
    ...
    CheckingTheModel
    write.table(x = CheckingTheModel,file = "clipboard",sep = "\t")
    ...
    VALUES ('LTV', 'LTV - New Script', 'Create the Models - LTV New Script', 'Create the Models script ran successfully',1,'2017-01-02 00:00:00.000');"
  ...
  ,error=function(err){
    ...
    Alerts_conn<-odbcDriverConnect('SERVER=localhost;DRIVER={SQL Server};DATABASE=Predict_DB;trusted_connection=true')
    QueryString<-"INSERT INTO [Predict_DB].[dbo].Alerts (MasterModel, Model, Step,Description,Successful,DateRunnigTheStep)
    VALUES ('LTV', 'LTV - New Script', 'Create the Models - LTV New Script', 'Create the Models script not ran successfully',0,'2017-01-02 00:00:00.000');"
    sqlQuery(channel = Alerts_conn,
             query = gsub(x = QueryString,pattern = '2017-01-02 00:00:00.000',replacement = as.character.Date(Sys.time())))
    odbcClose(Alerts_conn)
  }
